...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(prog='anpr.py')
    parser.add_argument('--weights', type=str, default='weights/best.pt', help='path to weights for plate detection model')
    parser.add_argument('--device', type=str, default='cpu', help='device to run on')
    parser.add_argument('--savepath', type=str, default='runs/recognize', help='save results path')  
    parser.add_argument('--path2detect', type=str, default='./to_detect', help='path to images and videos for recognition')
    parser.add_argument('--imagename', type=str, default='*.jpg', help='path to image for recognition')
    parser.add_argument('--videoname', type=str, default='*.mp4', help='path to video for recognition')  
    parser.add_argument('--imagesize', type=int, default=640, help='images size')
    parser.add_argument('--trace', type=bool, default=True, help='trace the code')
    parser.add_argument('--detecttype', type=str, default='image', help='file type to recognize: image, video, webcam')
    opt = parser.parse_args()
    
    images_n_vids_path = opt.path2detect
    image_path = os.path.join(images_n_vids_path, opt.imagename)
    video_path = os.path.join(images_n_vids_path, opt.videoname)
    savepath = opt.savepath
    weights = opt.weights
    device_id = opt.device
    image_size = opt.imagesize
    trace = opt.trace
    detecttype = opt.detecttype
